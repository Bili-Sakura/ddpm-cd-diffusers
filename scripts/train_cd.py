"""
Change Detection training and testing script.

Uses a pre-trained DDPM as a feature extractor and trains a change detection
head (cd_head_v2) on the extracted features.

Usage:
    python scripts/train_cd.py -c configs/levir.json -p train
    python scripts/train_cd.py -c configs/levir.json -p test
"""
...
from libs.print_diffuse_feats import print_feats
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-c', '--config', type=str, default='configs/levir.json',
        help='JSON file for configuration'
    )
    parser.add_argument(
        '-p', '--phase', type=str, choices=['train', 'test'],
    ...
    # Load change-detection datasets
    ...
    # Load diffusion model (feature extractor)
    ...
    if opt['phase'] == 'train':
        for current_epoch in range(start_epoch, n_epoch):
            ...
            for current_step, train_data in enumerate(train_loader):
                ...
                if current_step % opt['train']['train_print_freq'] == 0:
                    ...
                    message = (
                        '[Training CD]. epoch: [%d/%d]. Itter: [%d/%d], '
                        'CD_loss: %.5f, running_mf1: %.5f\n'
                    ) % (
                        current_epoch, n_epoch - 1, current_step,
                        len(train_loader), logs['l_cd'], logs['running_acc']
                    )
                    logger.info(message)
                    ...
                    img_mode = 'grid'
                    if img_mode == 'single':
                        img_A = Metrics.tensor2img(train_data['A'], out_type=np.uint8, min_max=(-1, 1))
                        img_B = Metrics.tensor2img(train_data['B'], out_type=np.uint8, min_max=(-1, 1))
                        gt_cm = Metrics.tensor2img(
                            visuals['gt_cm'].unsqueeze(1).repeat(1, 3, 1, 1),
                            out_type=np.uint8, min_max=(0, 1)
                        )
                        pred_cm = Metrics.tensor2img(
                            visuals['pred_cm'].unsqueeze(1).repeat(1, 3, 1, 1),
                            out_type=np.uint8, min_max=(0, 1)
                        )
                        Metrics.save_img(img_A, '{}/img_A_e{}_b{}.png'.format(train_result_path, current_epoch, current_step))
                        Metrics.save_img(img_B, '{}/img_B_e{}_b{}.png'.format(train_result_path, current_epoch, current_step))
                        Metrics.save_img(pred_cm, '{}/img_pred_e{}_b{}.png'.format(train_result_path, current_epoch, current_step))
                        Metrics.save_img(gt_cm, '{}/img_gt_e{}_b{}.png'.format(train_result_path, current_epoch, current_step))
                    else:
                        ...
                        Metrics.save_img(
                            grid_img,
                            '{}/img_A_B_pred_gt_e{}_b{}.png'.format(
                                train_result_path, current_epoch, current_step
                            )
                        )
            ...
            if current_epoch % opt['train']['val_freq'] == 0:
                ...
                for current_step, val_data in enumerate(val_loader):
                    ...
                    if current_step % opt['train']['val_print_freq'] == 0:
                        logs = change_detection.get_current_log()
                        message = (
                            '[Validation CD]. epoch: [%d/%d]. Itter: [%d/%d], '
                            'running_mf1: %.5f\n'
                        ) % (
                            current_epoch, n_epoch - 1, current_step,
                            len(val_loader), logs['running_acc']
                        )
                        logger.info(message)
                        ...
                        img_mode = 'grid'
                        if img_mode == 'single':
                            img_A = Metrics.tensor2img(val_data['A'], out_type=np.uint8, min_max=(-1, 1))
                            img_B = Metrics.tensor2img(val_data['B'], out_type=np.uint8, min_max=(-1, 1))
                            gt_cm = Metrics.tensor2img(
                                visuals['gt_cm'].unsqueeze(1).repeat(1, 3, 1, 1),
                                out_type=np.uint8, min_max=(0, 1)
                            )
                            ...
                            Metrics.save_img(img_A, '{}/img_A_e{}_b{}.png'.format(val_result_path, current_epoch, current_step))
                            Metrics.save_img(img_B, '{}/img_B_e{}_b{}.png'.format(val_result_path, current_epoch, current_step))
                            Metrics.save_img(pred_cm, '{}/img_pred_e{}_b{}.png'.format(val_result_path, current_epoch, current_step))
                            Metrics.save_img(gt_cm, '{}/img_gt_e{}_b{}.png'.format(val_result_path, current_epoch, current_step))
                        else:
                            visuals['pred_cm'] = visuals['pred_cm'] * 2.0 - 1.0
                            visuals['gt_cm'] = visuals['gt_cm'] * 2.0 - 1.0
                            grid_img = torch.cat((
                            ...
                            Metrics.save_img(
                                grid_img,
                                '{}/img_A_B_pred_gt_e{}_b{}.png'.format(
                                    val_result_path, current_epoch, current_step
                                )
                            )
                ...
                if logs['epoch_acc'] > best_mF1:
                    ...
                    logger.info(
                        '[Validation CD] Best model updated. '
                        'Saving the models (current + best) and training states.'
                    )
                else:
                    is_best_model = False
                    logger.info(
        ...
        for current_step, test_data in enumerate(test_loader):
            ...
            if img_mode == 'single':
                ...
                Metrics.save_img(
                    grid_img,
                    '{}/img_A_B_pred_gt_{}.png'.format(test_result_path, current_step)
                )
